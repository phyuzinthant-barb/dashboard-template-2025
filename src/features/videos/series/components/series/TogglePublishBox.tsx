...
>(({ setEditData, editData }, ref) => {
  const SeriesSchema = z.object({
    status: z.string().min(1, "Plan is required"),
  });
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className=" text-lg text-center">
            Do you want to{" "}
            {editData?.status == "PUBLISHED" ? "Unpublish" : "Publish"} this
            Series?
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <div className="flex gap-3 items-center justify-center">
          ...
          <Button
            ...
            onClick={() =>
              onSubmit({
                status:
                  editData?.status == "PUBLISHED" ? "UNPUBLISHED" : "PUBLISHED",
              })
            }
            type="submit"
          >
            {updating
              ? "Updating..."
              : `${editData?.status == "PUBLISHED" ? "Unpublish" : "Publish"}`}
          </Button>
        </div>
      </AlertDialogContent>
    </AlertDialog>
